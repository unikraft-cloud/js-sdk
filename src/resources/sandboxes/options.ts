...
export function pluginsFromSnapshot(
  ...
  throw new UnikraftCloudError(
    `The spec creates the sandbox from a template, a branch source, or a checkpoint, which already carries the sandbox plugin, so ${prose(passed, "and")} would attach it a second time, which the platform refuses. Drop ${passed.length === 1 ? "it" : "them"}. If the source attached the plugin under another name, pass that name as \`pluginName\`.`,
    { kind: "config" },
  );
}
 ...
 *
 * `ignore` names the keys a door does read, so it can refuse the rest. The
 * doors that borrow a client pass `client`, `metro` and `metros`: the borrow
 * is the point, and the metro still picks where the sandbox goes.
 *
 * @example
 ...
 *
 * @example
 * // Throws for the same reason, one door along.
 * await Sandbox.create({ image: "..." }, { client, token: MY_TOKEN });
 */
export function assertNoClientConfig(
  opts: object,
  door: string,
  ignore: readonly ClientOnlyOption[] = [],
): void {
  const record = opts as Record<string, unknown>;
  const named = CLIENT_ONLY_KEYS.filter(
    (key) => record[key] !== undefined && !ignore.includes(key),
  );
  ...
  remedies.push(
    ignore.includes("client")
      ? "drop `client`, so that these options build the client instead"
