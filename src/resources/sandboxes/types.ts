 ...
 *
 * A spec that names a `template`, a `branch_from` source, or a `checkpoint`
 * gets no default image, memory, or plugin entry: the snapshot carries all
 * three, and the platform refuses a `memory_mb` or a second sandbox plugin
 * next to it.
 */
