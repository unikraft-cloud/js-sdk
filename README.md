...
- **The SDK polls.** Where the platform has nothing to hold open, the SDK asks
  ...
  100 ms to 2 s with jitter, and the deadline is 60 s. The deadline and a
  `signal` both end the probe in flight, not only the loop around it. Only a
  failure that can still change is worth another probe — a network fault, or a
  `404`/`502`/`503`/`504`. A `401` or a `403` is rethrown at once, so a rejected
  token reports itself instead of timing out.
